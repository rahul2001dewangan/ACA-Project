...
/* RRIP: Find a block to replace using RRIP policy */
static struct cache_blk_t *
find_rrip_victim(struct cache_t *cp, struct cache_set_t *set)
{
    struct cache_blk_t *blk, *victim = NULL;
    int attempts = 0;
    const int max_attempts = cp->assoc * 2; // Reasonable upper bound

    // First, look for an invalid block (should be used first)
    ...
    // All blocks are valid, use RRIP
    while (victim == NULL && attempts < max_attempts) {
        // First pass: look for distant blocks
        for (blk = set->way_head; blk; blk = blk->way_next) {
            if ((blk->status & CACHE_BLK_VALID) && blk->rrpv == RRIP_DISTANT) {
                victim = blk;
                break;
            }
        }
        ...
        // Second pass: increment all RRPVs and look again
        for (blk = set->way_head; blk; blk = blk->way_next) {
            if (blk->status & CACHE_BLK_VALID) {
                if (blk->rrpv < RRIP_MAX) {
                    blk->rrpv++;
                }
                // Check if this block becomes distant after increment
                if (victim == NULL && blk->rrpv == RRIP_DISTANT) {
                    victim = blk;
                }
            }
        }
    ...
    // Final fallback: use LRU (tail) if still no victim
    if (victim == NULL) {
        victim = set->way_tail;
        // Emergency: if tail is invalid, find first valid block
        if (!(victim->status & CACHE_BLK_VALID)) {
            for (blk = set->way_head; blk; blk = blk->way_next) {
                if (blk->status & CACHE_BLK_VALID) {
                    victim = blk;
                    break;
	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  blk->tag = 0;
	  blk->ready = 0;
	  /* Initialize RRPV for RRIP policy */
	  blk->rrpv = (cp->policy == RRIP) ? RRIP_DISTANT : 0;
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  /* Set RRPV for new block - use RRIP_LONG for new insertions */
  if (cp->policy == RRIP)
    repl->rrpv = RRIP_LONG;
 ...
 cache_hit: /* slow hit handler */
  ...
  /* Update RRPV on hit for RRIP policy */
  if (cp->policy == RRIP)
    blk->rrpv = RRIP_NEAR_IMP;
	  ...
	  if (blk->status & CACHE_BLK_VALID)
	    {
	      ...
	      /* Reset RRPV for flushed blocks */
	      if (cp->policy == RRIP)
		blk->rrpv = RRIP_DISTANT;
